function New-CtxAutodeployVM {
    ...
    try {
        ...
        Write-VerboseLog -Message "High-level logging operation started with Id: '{Logging}'" -PropertyValues $Logging.Id
        ...
        if ($ProvisioningTask.TerminatingError) {
            Write-ErrorLog -Message "Machine provisioning task failed: $($ProvisioningTask.TerminatingError)" -PropertyValues $ProvisioningTask.TerminatingError
